...
export default function AdminAdsPage() {
  ...
  const [adsterraHeaderZone, setAdsterraHeaderZone] = useState('');
  const [adsterraNativeZone, setAdsterraNativeZone] = useState('');
  const [adsterraSidebarZone, setAdsterraSidebarZone] = useState('');
    ...
    async function loadConfig() {
      try {
        ...
        if (docSnap.exists()) {
          ...
          setAdsterraHeaderZone(adsterra.zoneIds?.header || '');
          setAdsterraNativeZone(adsterra.zoneIds?.native || '');
          setAdsterraSidebarZone(adsterra.zoneIds?.sidebar || '');
      ...
      await setDoc(docRef, {
        networks: {
          adsterra: {
            ...
            zoneIds: {
              header: adsterraHeaderZone,
              native: adsterraNativeZone,
              sidebar: adsterraSidebarZone,
            },
          },
          propellerads: {
        ...
        <div className="border-b border-neutral-200">
          <div className="max-w-4xl mx-auto px-6 py-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-neutral-500 mt-0.5">Toggle networks, set priorities, and configure zone keys.</p>
            </div>
          ...
          <form onSubmit={handleSave} className="space-y-8">
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-neutral-100">
                <h2 className="text-sm font-semibold text-black">Adsterra Settings</h2>
              ...
              {adsterraEnabled && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-label text-neutral-500">Header Zone Key</label>
                    <input
                      type="text"
                      value={adsterraHeaderZone}
                      onChange={(e) => setAdsterraHeaderZone(e.target.value)}
                      placeholder="e.g. 8d390bb9a69076..."
                      className="field-input"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-label text-neutral-500">Native Zone Key</label>
                    <input
                      type="text"
                      value={adsterraNativeZone}
                      onChange={(e) => setAdsterraNativeZone(e.target.value)}
                      className="field-input"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-label text-neutral-500">Sidebar Zone Key</label>
                    <input
                      type="text"
                      value={adsterraSidebarZone}
                      onChange={(e) => setAdsterraSidebarZone(e.target.value)}
                      className="field-input"
                    />
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-neutral-100">
                <h2 className="text-sm font-semibold text-black">PropellerAds Settings</h2>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
